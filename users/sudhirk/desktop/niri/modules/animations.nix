...
in
{
  programs.niri.config = ''
    ...
    binds {
        ...
        Mod+S { spawn-sh "niri msg action move-column-left && niri msg action focus-column-left && niri msg action focus-column-right"; }
        ...
        XF86MonBrightnessDown allow-when-locked=true { spawn-sh "brightnessctl --class=backlight set 4%-"; }
        XF86MonBrightnessUp allow-when-locked=true { spawn-sh "brightnessctl --class=backlight set +4%"; }
    }
  ...
  ''
  + (if hostName == "laptop" then laptopOutputs else "")
  + (if hostName == "desktop" then desktopOutputs else "");
}
